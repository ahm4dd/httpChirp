...
import { getUserFromRefreshToken, getRefreshTokenByToken, } from "./../db/queries/refreshTokens.js";
...
export async function handlerRefresh(req, res, next) {
    try {
        ...
        if (refreshToken.revokedAt !== null) {
            ...
            res.status(200).json({
                token: makeJWT(await getUserFromRefreshToken(refreshToken.token), 3600, config.serverApi),
            });
            res.end();
        }
    }
